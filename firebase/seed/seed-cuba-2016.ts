...
async function main() {
  ...
  const totalUSD =
    fSnap.docs.reduce((s, d) => s + ((d.data().price_usd as number) ?? 0), 0) +
    hSnap.docs.reduce((s, d) => s + ((d.data().total_price_usd as number) ?? 0), 0) +
    tSnap.docs.reduce((s, d) => s + ((d.data().price_usd as number) ?? 0), 0) +
    eSnap.docs.reduce((s, d) => s + ((d.data().amount_usd as number) ?? 0), 0);

  const paidUSD =
    fSnap.docs.reduce((s, d) => s + ((d.data().paid_amount as number) ?? 0), 0) +
    hSnap.docs.reduce((s, d) => s + ((d.data().paid_amount as number) ?? 0), 0) +
    tSnap.docs.reduce((s, d) => s + ((d.data().paid_amount as number) ?? 0), 0) +
    eSnap.docs.reduce((s, d) => s + ((d.data().paid_amount as number) ?? 0), 0);
